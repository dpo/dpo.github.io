...
TOP=$(PWD)
BIB_SRC="${HOME}/tex/vita/Publications"
BIB_LOCATION="${TOP}/publications"
...
function convert_preprint_entries() {
    for bibfile in $(ls ${BIB_SRC}/articles*.bib ${BIB_SRC}/techrep*.bib ${BIB_SRC}/books*.bib ${BIB_SRC}/proceedings*.bib ${BIB_SRC}/unpublished*.bib); do
        convert_preprint_entry $bibfile
    done
}

function convert_preprint_entry() { # arg1 = bibfile
    sed -e 's/^[[:space:]]*[Pp]reprint[[:space:]]*=/annote =/g' $1 >${BIB_LOCATION}/$(basename $1)
}

function main() {
    cd $BIB_LOCATION
    convert_preprint_entries
    begin_yaml_header >$PUB_HEADER
    echo "" >$PUB_BODY
    for year in $(seq $(date +"%Y") 2000); do
